...
class TradesManager {
    ...
    async fetchLivePrices() {
        try {
            const symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT', 'DOGEUSDT', 'BNBUSDT', 'ADAUSDT'];
            ...
            if (response.ok) {
                ...
                const targetSym = `${this.currentSymbol || 'BTC'}USDT`;
                const found = data.find(item => item.symbol === targetSym);
                if (found) {
                    this.currentPrice = parseFloat(found.price);
                    const priceEl = document.getElementById('currentPrice');
                    if (priceEl) {
                        const formatted = this.currentPrice < 1 ? this.currentPrice.toFixed(4) : this.currentPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            ...
            console.error('Failed to fetch live prices from Binance:', err);
        }
    }
    ...
    startPriceUpdates() {
        ...
        this.priceUpdateInterval = setInterval(() => this.fetchLivePrices(), 3000);
    }
