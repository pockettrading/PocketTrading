...
class MarketsManager {
    ...
    renderMarkets() {
        ...
        tbody.innerHTML = filtered.map(m => `
            <tr id="market-row-${m.symbol}">
                ...
                <td style="font-weight: 600;" id="price-${m.symbol}">$${this.formatPrice(m.price)}</td>
                <td class="${m.change_24h >= 0 ? 'positive' : 'negative'}" style="font-weight: 600;" id="change-${m.symbol}">
                    ${m.change_24h >= 0 ? '▲' : '▼'} ${Math.abs(m.change_24h).toFixed(2)}%
                </td>
                <td id="vol-${m.symbol}">${m.volume || '—'}</td>
                <td>
    ...
    updateMarketStats() {
        ...
        if (lastUpdated) {
            lastUpdated.innerHTML = `Live Feed: Binance API • ${new Date().toLocaleTimeString()}`;
        }
    }
    ...
    async loadMarkets() {
        try {
            ...
            this.renderMarkets();
            this.updateMarketStats();
            // Trigger live price update immediately after loading
            this.fetchLiveBinancePrices();
        } catch (error) {
    ...
    startPriceUpdates() {
        // Refresh live prices from Binance every 3 seconds
        this.priceUpdateInterval = setInterval(() => {
            this.fetchLiveBinancePrices();
        }, 3000);
    }

    async fetchLiveBinancePrices() {
        if (!this.allMarkets || this.allMarkets.length === 0) return;
        
        const symbols = this.allMarkets.map(m => `"${m.symbol.toUpperCase()}USDT"`);
        ...
        try {
            ...
            this.allMarkets = this.allMarkets.map(m => {
                const t = tickerMap[m.symbol.toUpperCase()];
                if (!t) return m;
                ...
                if (!isNaN(qVol) && qVol > 0) {
                    if (qVol >= 1e9) newVol = `${(qVol / 1e9).toFixed(1)}B`;
                    else if (qVol >= 1e6) newVol = `${(qVol / 1e6).toFixed(0)}M`;
                    else newVol = `$${qVol.toLocaleString(undefined, {maximumFractionDigits:0})}`;
                }
                
                // Update DOM directly if cell exists
                const priceEl = document.getElementById(`price-${m.symbol}`);
                const changeEl = document.getElementById(`change-${m.symbol}`);
                const volEl = document.getElementById(`vol-${m.symbol}`);
                
                if (priceEl && newPrice !== oldPrice && !isNaN(newPrice)) {
                ...
                if (changeEl && !isNaN(newChange)) {
                    changeEl.textContent = `${newChange >= 0 ? '▲' : '▼'} ${Math.abs(newChange).toFixed(2)}%`;
                    changeEl.className = newChange >= 0 ? 'positive' : 'negative';
                    changeEl.style.fontWeight = '600';
                }
                
                if (volEl) volEl.textContent = newVol;
                
                statsUpdated = true;
                return {
                    ...m,
                    price: isNaN(newPrice) ? m.price : newPrice,
                    change_24h: isNaN(newChange) ? m.change_24h : newChange,
                    volume: newVol
                };
            });
            ...
            console.error('Error fetching live Binance prices:', error);
        }
    }
