...
class AdminManager {
    ...
    async init() {
        ...
        if (this.currentUser && this.currentUser.id) {
            try {
                dbUser = await supabaseDB.getUserById(parseInt(this.currentUser.id));
            } catch(e) { console.error('Admin DB check error:', e); }
        } else {
            const userId = sessionStorage.getItem('pocket_user_id') || localStorage.getItem('pocket_user_id');
            if (userId) {
                try {
                    dbUser = await supabaseDB.getUserById(parseInt(userId));
                } catch(e) {}
            }
        }
        
        if (!dbUser || (dbUser.email !== 'ephremgojo@gmail.com' && dbUser.is_admin !== true)) {
        ...
        if (typeof auth !== 'undefined') auth.currentUser = dbUser;
